...
class RecordDB(
    ...
    def get_records(self, uids, chunksize = None,):
        ...
        self.cursor.execute(f'create table temp_idx ([index_] varchar(10) not null primary key)')  # create temporary table
        ...
        if self.var_list is None:
            data = pd.read_sql(
                f"select * from temp_idx as t1 inner join {self.table_name} as t2 on (t1.[index_]=t2.{self.idx_name}) {self.extra_joins}",
